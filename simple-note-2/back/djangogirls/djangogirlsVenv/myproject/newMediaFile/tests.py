...
class NewMediaFileViewTest(APITestCase):
    def setUp(self):
        ...
        self.valid_file_content = io.BytesIO(b"Test file content")  # 模擬的文件內容（以二進位形式）
    ...
    @patch("db_modules.UserFileData.check_file_name")  # 模擬檢查文件名稱是否存在的方法
    @patch("db_modules.SaveFile.SaveFile.renameAganistDumplication")  # 模擬文件名稱重命名方法
    def test_post_duplicate_file_name(self, mock_rename_against_duplication, mock_check_file_name):
        ...
        with patch("db_modules.UserFileData.insert_file_name") as mock_insert_file_name, \
            ...
            mock_rename_against_duplication.assert_called_once_with(self.valid_payload["filename"])  # 確認檔案名稱重命名方法被調用一次
