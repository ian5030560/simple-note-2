...
class User_Personal_Info(Base):
    ...
    def __init__(self,id,profile_photo,theme_id,usernames,user_email,user_password,login_status):
        self.id = id
...
def check_username_password(username, password):
    user = session.query(User_Personal_Info).filter_by(usernames=username, user_password=password).first()
    if user:
        return True
    else:
        return False
...
# def insert_username_password_email(username,password):             

print(check_email("user01@gmail.com"))       
