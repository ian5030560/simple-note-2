...
class LoadNoteTreeViewTests(APITestCase):
    ...
    @patch(
        "db_modules.UserNoteData.check_user_all_notes"
    )  # 模擬check_user_all_notes方法
    ...
    def test_post_valid_single_notes(
        self, mock_check_sibling_id, mock_check_parent_id, mock_check_user_all_notes
    ):
        """測試POST請求，處理有效的單一筆記資料"""

        # 模擬用戶的筆記資料
        mock_check_user_all_notes.return_value = [("Note1", "ID1"), ("Note2", "ID2")]
    ...
    @patch(
        "db_modules.UserCollaborateNote.check_all_noteID_by_guest"
    )  # 模擬check_all_noteID_by_guest方法
    @patch(
        "db_modules.UserNoteData.check_note_title_id_by_note_id"
    )  # 模擬check_note_title_id_by_note_id方法
    @patch(
        "db_modules.UserNoteData.check_note_name_by_note_id"
    )  # 模擬check_note_name_by_note_id方法
    def test_post_valid_collaboration_notes(
        self,
        mock_check_note_name,
        mock_check_note_title_id,
        mock_check_all_noteID_by_guest,
        mock_check_url,
        mock_check_user_all_notes,
    ):
        ...
        # 檢查回應資料是否與預期相符
        self.assertEqual(response.json(), {"one": [], "multiple": []})
    ...
    def test_post_invalid_request_body(self):
        ...
        # 發送無效的POST請求
        response = self.client.post(
            self.url, json.dumps(self.invalid_payload), content_type="application/json"
        )
